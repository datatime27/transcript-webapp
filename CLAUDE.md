...
- `get_active_users()` — returns active users (`WHERE active = 1`) including `wants_more`, `active`, `episodes_assigned` (COUNT DISTINCT from `user_episodes`), `episodes_started` (COUNT DISTINCT episode_uid from `versions`), and `created_at` (ISO string with `America/New_York` offset) via a single query with two LEFT JOINs.
...
- `add_episode_to_user(user_uid, episode_uid)` — inserts into `user_episodes` and clears `wants_more`. Raises `ValueError` (→ 409) if the episode has a completed v2.0 version and another user is currently active (non-complete).
...
- `get_wants_more_suggestions()` — for each user with `wants_more=1`, returns the best unassigned episode from their season (via locations table, US fallback). Excludes episodes assigned to admin/test users from the count. Skips episodes that are locked (have any v2.0 activity and any non-complete assignment). Returns `{user_uid: {episode_uid, show_name, season_number, episode_number} | None}`. Uses 4 queries total regardless of user count.
