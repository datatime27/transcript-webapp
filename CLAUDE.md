...
4. Diff table spans full window width. Layout: YouTube player above (default 40% height), draggable 5px horizontal splitter (row-resize, constrained 10–90%), table below. Drag shield overlays iframe during drag. Shows one row per caption (union across all versions, keyed by raw `String(cap.start)`). Column order: Time | Text | Result | [per-user columns]. Agreed rows are greyed; disagreed rows are highlighted with per-user clickable speaker cells and `M` badges for `modified=true`. Missing captions show "—". Text conflicts: `col-text` cell turns red (`col-text text-conflict` class); per-version text shown as a red `.text-snippet` span below each speaker cell (outside the speaker-cell span). Both share common styles via grouped selector (color, font-weight, vertical-align, word-wrap); `.text-snippet` adds `display:block` and `margin-top`. Caption text is never truncated — wraps fully. `refreshRow` targets the result cell via `td.col-result` (not positional index) since user columns follow it.
