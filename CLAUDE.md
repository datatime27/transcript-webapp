...
- **User Latency** — table of non-complete user-episode assignments sorted oldest activity first (excludes admin, test, and complete-season rows). Columns: User (links to `viewer.html?user=uid&episode=uid`), Episode, Last Activity, Age (orange ≥14 days, red ≥30 days), Status (version count or "Not started"), Latest Modification (timestamp of last `modified=true` caption in the JSON), % Modified (percentage of captions with `modified=true`). Both file-derived columns are populated for the oldest 10 rows only. `get_user_latency()` fetches `latest_filepath` via a correlated subquery; `_file_stats(filepath)` in `admin.py` opens the file once and returns both `(latest_modification, percent_modified)`.
