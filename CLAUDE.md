...
- **Episodes** — all episodes with user version tags, grouped by season with `<div class="season-separator">` headings. Started tags link to `reapply.html`; unstarted tags link to `viewer-2.0.html?user=...&episode=...`. Admin users and test users are hidden from unstarted tags (only appear once they've saved a version). Each tag shows the user's name, version number, and a `· Mon D` date (latest save timestamp, or assignment timestamp for unstarted users). Tags are sorted by timestamp ascending. Completed v2.0 tags (user finished via viewer-2.0.html) have a teal `.ready-review` border. Episodes with a completed v2.0 annotator and an active non-complete reviewer show a 🔒 locked badge. Each season heading has a **Mark complete** / **Mark active** toggle button: clicking it POSTs `set_season_complete` with `{season_uid, is_complete}` and re-renders the list — completed seasons collapse into a "Completed seasons" disclosure at the bottom. `get_episodes_with_user_versions()` now includes `season_uid` and `latest_app_version` per user in each episode dict.
...
`admin.py` POST actions: `delete_test_accounts`, `create_user`, `populate_transcript`, `add_episode_to_user`, `set_season_speakers`, `set_season_complete`, `set_location_season`, `update_user_location`. Each action is a module-level function returning `(status, body)`; dispatched via `POST_ACTIONS` dict.
