...
- **Episodes** — all episodes with user version tags, grouped by season with `<div class="season-separator">` headings. Season headings are collapsible (click to toggle; all start collapsed); an **Expand all** button in the heading expands all active seasons at once. Started tags link to `reapply.html`; unstarted tags link to `viewer-2.0.html?user=...&episode=...`. All users (including admin and test) are shown regardless of whether they've started. Each tag shows the user's name, version number, and a `· Mon D` date (latest save timestamp, or assignment timestamp for unstarted users). Tags are sorted by timestamp ascending. Completed v2.0 tags (user finished via viewer-2.0.html) have a teal `.ready-review` border. Episodes with a completed v2.0 annotator and an active non-complete reviewer show a 🔒 locked badge. Each season heading has a **Mark complete** / **Mark active** toggle button: clicking it POSTs `set_season_complete` with `{season_uid, is_complete}` and re-renders the list — completed seasons collapse into a "Completed seasons" disclosure at the bottom. `get_episodes_with_user_versions()` now includes `season_uid` and `latest_app_version` per user in each episode dict. Unstarted user tags have an **× button** (`.unassign-btn`) that POSTs `remove_episode_from_user` and removes the tag from the DOM on success; started tags (with saved versions) have no × button.
...
`add_episode_to_user` POST action: accepts `{user_uid, episode_uid}`. Returns 409 if the episode is locked (v2.0 episode with an active non-complete reviewer). After assigning, emails the user — welcome email (with uppercased name) if it's their first episode, otherwise a "new episode ready" email with a direct link. The welcome email uses `viewer-2.0.html?user={uid}` (no episode param); subsequent emails use `viewer-2.0.html?user={uid}&episode={episode_uid}`.
...
- `get_wants_more_suggestions()` — for each user with `wants_more=1`, returns the best unassigned episode from their season (via locations table, US fallback). Excludes episodes assigned to admin/test users from the count. Skips episodes that are locked (have a completed v2.0 version and an active non-complete reviewer). Returns `{user_uid: {episode_uid, show_name, season_number, episode_number} | None}`. Uses 4 queries total regardless of user count.
