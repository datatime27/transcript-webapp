...
Admin-only tool at `admin.html?user={uid}`. Sections (in order):
- **Recent Versions** — latest version per user per episode in the last 7 days; columns: User, Episode, Version, Done (✓ if `is_complete`), Saved at. Timestamps stored as Eastern Time in MySQL, sent as ISO with `America/New_York` offset via `zoneinfo`, formatted in the browser via `new Date(...).toLocaleString()`.
...
- `get_recent_versions()` — latest version per user per episode from the last 7 days; JOINs `user_episodes` to include `is_complete`; timestamps use `zoneinfo` `America/New_York`.
